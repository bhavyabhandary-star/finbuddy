...
| Hindi language toggle (customer PWA only) | **Real, scoped.** The "Ask FinBuddy" chat has a real English/हिंदी toggle. Retrieval stays English-only (no separately-verified Hindi corpus); in Hindi mode, Groq is instructed to translate the same English-verified context on the fly -- that translation's fidelity for compliance-critical figures is NOT independently verified the way the English answers are. The low-confidence escalation message in Hindi is a fixed, human-written string (not LLM-translated), so that specific path is as reliable as the English one. Not wired to WhatsApp (scoped decision, not a limitation of the underlying code). |
