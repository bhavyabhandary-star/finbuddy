...
| Fairness Audit | Risk & Compliance | F-012, precisely (correcting an earlier imprecise version of this row): the **deployed** mitigation (`ThresholdOptimizer` fit jointly on geography × income_band) achieves DPD ≤0.05 and DIR ≥0.80 for **geography and income_band** on the deployed model (DPD 0.0053 / 0.0225, DIR 0.99 / 0.96). **Gender is NOT part of that joint fit** — under the actual deployed model, gender's DPD is 0.1168 (fails 0.05) and EOD 0.1239 (fails), though DIR 0.81 passes. A *separate, not-deployed* isolated demo proves gender's gap IS fixable (all three metrics pass when gender is mitigated alone: true base rates are ~equal across gender, unlike geography/income_band) — included in the audit for completeness, not shipped. EOD ≤0.05 for geography/income_band is **not mathematically achievable** at their base-rate gap (Kleinberg/Chouldechova) — a human sign-off item, not an automated pass. See `f012_fairness_audit.json`'s `gender_on_deployed_model` and `gender_isolated_demo` keys for the exact figures. |
